...
$EmailParameters = @{
    EmailFrom                   = "no-reply@horizonind.com"
    EmailTo                     = "apenick@horizonind.com"
    EmailCC                     = ""
    EmailBCC                    = ""
    EmailReplyTo                = ""
    EmailServer                 = "smtp.domain.local"
    EmailServerPassword         = ""
    EmailServerPasswordAsSecure = $false
    EmailServerPasswordFromFile = $false
    EmailServerPort             = "25"
    EmailServerLogin            = ""
    EmailServerEnableSSL        = 0
...
$FormattingParameters = @{
    CompanyBranding        = @{
        Logo   = '\\fileserv\S\Public\Sales_Marketing\Logos\Horizon\New-Horizon-Logo-No-Text.png'
        Width  = '200'
        Height = ''
        Link   = 'https://www.horizonind.com'
        Inline = $false
    }
...
$ReportOptions = @{
    ...
    SendMail              = $true
    OpenAsFile            = $false
    KeepReports           = $false # keeps files after reports are sent (only if AssExcel/AsCSV are in use)
    KeepReportsPath       = "C:\Support\Reports\ExportedEvents" # if empty, temp path is used
    FilePattern           = "Horizonind-<currentdate>.<extension>"
